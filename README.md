...
| `npm run details` | Fetches each template's README summary and dependency stack |
...
A template page carrying only its title and one-line description has around **seventeen words** of
its own. That is not enough for a search engine to treat it as a page about anything, and not
enough for a reader to choose with — which matters, because the whole search strategy rests on
those 418 pages.
...
Two calls per template — its README and its `package.json` — producing:

- **What's inside.** Real dependencies grouped by what they are for: framework, styling,
  components, database, auth, payments, content, testing. A major version is printed only when it
  distinguishes something; "Next.js 16" is worth saying, "Playwright 1" is not.
- **About this template.** The opening of the README, cleaned of the badge rows, centred HTML,
  logos and comparison tables that READMEs open with — otherwise the page quotes "npm version
  build passing license MIT" as its description. Plus the bullets from a Features section, if it
  has one.

The words are the authors', so the section says so and links back to the source. Kept in
`data/details.json` keyed by template id, for the same reason as screenshots: an ingest run
rewrites `templates.json` wholesale. The weekly ingest workflow runs this straight after, so a
newly discovered template arrives with its page already filled in.
...
lib/
  ...
  details.ts            README and package.json -> summary, features, stack
...
scripts/
  ...
  details.ts            fetches READMEs and manifests
data/
  ...
  details.json          generated by details, keyed by template id
